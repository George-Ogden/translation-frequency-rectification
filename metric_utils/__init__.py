...
def fid(paths, batch_size=50, device=None, dims=2048):
    ...
    files = map(glob, paths)
    block_idx = inception.InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = inception.InceptionV3([block_idx]).to(device)
    fake_statistics, real_statistics = [fid_score.calculate_activation_statistics(file_list, device=device, model=model, batch_size=batch_size, dims=dims) for file_list in files]
    return fid_score.calculate_frechet_distance(*fake_statistics, *real_statistics)
